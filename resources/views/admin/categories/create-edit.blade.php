...
<div class="p-8">
    ...
    <form method="post" @if($category) action="/admin/categories/{{$category->id}}" @else  action="/admin/categories" @endif enctype="multipart/form-data">
        ...
        <div class="flex flex-col gap-6 mb-6 md:grid-cols-2 w-full">
            ...
            <div class="flex flex-row gap-5">
                <div class="w-full">
                    <label class="block mb-2 text-sm font-medium text-gray-900 " for="file_input">@if($category->icon_url ?? null) Change @endif Category Icon</label>
                    <input name="icon" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" type="file">
