...
<form class="w-full h-full rounded-md border-[1px] border-[#707070] overflow-hidden" @if($product->variants->count() <= 1) action="/cart/add" method="post" @endif>
        ...
        <div class="flex items-center justify-center p-2 mt-3 ml-2 flex-col-2">
            <button type="button" data-favourite="{{$product->id}}" class="{{$product->available_on_favourite ? 'fav_available' : 'favourite_btn'}} w-2/4 p-2 text-white border-2 rounded-sm {{$product->available_on_favourite ? 'bg-gray-600' : 'bg-blue-500'}}" {{$product->available_on_favourite ? 'disabled' : ''}}>
                My List
            </button>
            @if($product->in_stock)
            @if($product->variants->count() == 1)
            <input type="hidden" name="variant_id" value="{{$product->variants[0]->id}}">
            <button type="submit" class="w-3/4 p-2 ml-4 bg-gradient-to-b from-[#166EB6] to-[#284297] rounded-sm text-white">
                ADD TO CART
            </button>
            @else
            <a href="/products/{{$product->id}}" class="w-3/4 p-2 ml-4 bg-gradient-to-b from-[#166EB6] to-[#284297] rounded-sm text-center text-white">
                VIEW OPTIONS
            </a>
            @endif
            @else
