<form action="/products" class="my-auto"    >
    <div class="flex border-2 border-white rounded-lg overflow-hidden">
        <select name="category_id" id="dropdown-button" type="button" class="inline-flex max-w-[75px] justify-center bg-white px-3 py-3 text-sm font-semibold text-gray-900 shadow-sm" id="menu-button" aria-expanded="true" aria-haspopup="true">
            <option value="">All</option>
            @foreach($categories as $category)
            <option value="{{$category->id}}" @if(($_GET['category_id'] ?? null) == $category->id) selected @endif>{{$category->name}}</option>
            @endforeach
        </select>
        <input name="q" value="{{$_GET['q'] ?? null}}" class="px-2.5 py-2 bg-transparent text-white focus:outline-none" type="text" placeholder="" />
